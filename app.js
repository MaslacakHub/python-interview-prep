...
let questionLibraries = {
    'main': {
        name: 'Main Library',
        description: '1000 General Python Interview Questions',
        questions: questions
    }
};
...
// Load question libraries
function loadQuestionLibraries() {
    // Check if data science library is loaded
    if (typeof dataScienceQuestions !== 'undefined') {
        questionLibraries['data-science'] = {
            name: dataScienceQuestions.libraryName,
            description: dataScienceQuestions.description,
            questions: dataScienceQuestions.questions
        };
        // Add to all questions
        allQuestions = [...allQuestions, ...dataScienceQuestions.questions];
    }
    
    // Check if vega interview library is loaded
    if (typeof vegaInterviewQuestions !== 'undefined') {
        questionLibraries['vega-interview'] = {
            name: vegaInterviewQuestions.libraryName,
            description: vegaInterviewQuestions.description,
            questions: vegaInterviewQuestions.questions
        };
        // Add to all questions
        allQuestions = [...allQuestions, ...vegaInterviewQuestions.questions];
    }
...
function populateLibrarySelector() {
    ...
    while (librarySelector.children.length > 1) {
        librarySelector.removeChild(librarySelector.lastChild);
    }
    
    // Add each library
    Object.keys(questionLibraries).forEach(libKey => {
        const lib = questionLibraries[libKey];
        const option = document.createElement('option');
        option.value = libKey;
        option.textContent = `${lib.name} (${lib.questions.length} questions)`;
...
function init() {
    ...
    document.getElementById('library-selector').addEventListener('change', filterByLibrary);
}
...
function populateCategoryFilter() {
    ...
    // Get categories from currently selected questions
    const currentQuestions = getCurrentQuestionSet();
...
// Get current question set based on library selection
function getCurrentQuestionSet() {
    if (selectedLibrary === 'all') {
        return allQuestions;
    } else if (questionLibraries[selectedLibrary]) {
        return questionLibraries[selectedLibrary].questions;
    }
    return allQuestions;
}
...
function filterByLibrary() {
    selectedLibrary = document.getElementById('library-selector').value;
    
    // Reset category filter
    document.getElementById('category-filter').value = 'all';
    selectedCategory = 'all';
    
    // Update questions based on library
    if (selectedLibrary === 'all') {
        shuffledQuestions = [...allQuestions];
    } else if (questionLibraries[selectedLibrary]) {
        shuffledQuestions = [...questionLibraries[selectedLibrary].questions];
    } else {
        shuffledQuestions = [...allQuestions];
    }
    ...
    if (shuffledQuestions.length > 0) {
        ...
        document.getElementById('question-text').textContent = 'No questions found in this library.';
    }
}
...
function filterByCategory() {
    ...
    // Get base question set from library selection
    const baseQuestions = getCurrentQuestionSet();
    
    if (selectedCategory === 'all') {
        shuffledQuestions = [...baseQuestions];
    } else {
        shuffledQuestions = baseQuestions.filter(q => q.category === selectedCategory);
    }
